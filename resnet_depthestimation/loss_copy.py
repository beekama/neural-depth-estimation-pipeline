...
class MonodepthLoss(nn.modules.Module):
    ...
    def disp_smoothness(self, disp, pyramid):
        ...
        #                 for i in range(self.n)]
        smoothness_x = [torch.abs(disp_gradients_x[i])
                        for i in range(self.n)]
        smoothness_y = [torch.abs(disp_gradients_y[i])
                        for i in range(self.n)]
    ...
    def forward(self, input, target):
        ...
        # Convert the input (disparity) to 3 channels
        input_disp = torch.cat([input, input, input], dim=1)

        # Prepare disparities
        disp_left_est = [input_disp for _ in range(self.n)]
        disp_right_est = [input_disp for _ in range(self.n)]

        # Generate images
        left_est = [self.generate_image_left(right, disp_left_est[i]) for i in range(self.n)]
        right_est = [self.generate_image_right(left, disp_right_est[i]) for i in range(self.n)]
        ...
        # Disparities smoothness
        disp_left_smoothness = self.disp_smoothness(disp_left_est, [left for _ in range(self.n)])
        disp_right_smoothness = self.disp_smoothness(disp_right_est, [right for _ in range(self.n)])
    ...
    ##todo: rewrite for input gray schon bei orino, 